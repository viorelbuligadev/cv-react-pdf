...
export const posts: BlogPost[] = [
  {
    ...
    faq: [
      ...
      { q: 'Does calling AsQueryable() on a List give me SQL translation?', a: 'No. AsQueryable on an in-memory collection wraps it in a queryable facade whose provider still executes everything in memory. There is no database behind it and no SQL is generated. It is useful for satisfying a method signature, never for performance. And be careful using it to fake a DbSet in tests: an EnumerableQuery has no async query provider, so any async EF Core operator - ToListAsync, FirstOrDefaultAsync, CountAsync - throws at runtime, which is why the EF Core testing docs discourage mocking DbSet and point to SQLite in-memory or your real database instead.' },
